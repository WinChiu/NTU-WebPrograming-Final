    ...
    <>
      ...
      >
        ...
        <span className={imgs.imageButton}>
          <Typography component="span" variant="subtitle1" color="inherit" className={imgs.imageTitle}>
            {props.note.title}
            <span className={imgs.imageMarked} />
          </Typography>
        </span>
        <span className={imgs.text}>
          <div className={imgs.texttitle}>{props.note.title}</div>
          ...
          <div className={imgs.textgrade}>年級 : {props.note.grade}</div>
          <div className={imgs.textsubject}>{props.note.subject}</div>
          <div className={imgs.texthassold}>觀看次數 : {hassold}</div>
          <div className={imgs.textprice}>售價 : {props.note.price}</div>
        </span>
      </ButtonBase>
      <Modal
        title={"購買筆記"}
        ...
        height="600px"
        onOk={handleBuy}
        okButtonProps={haveBuy === "own" || haveBuy === "buy" ? { disabled: true } : { disabled: false }}
      ...
      >
        <div className="confirm">
          <div className="confrim_photo">
            <Image.PreviewGroup>
              <Image width={200} src={props.note.img ? props.note.img : noteimg} />
            </Image.PreviewGroup>
          </div>
          <div className="confirm_buy">
            <p className="modal_texttitle">{props.note.title}</p>
            ...
            <div className="modal_textgrade">年級 : {props.note.grade}</div>
            <div className="modal_textsubject">科目 : {props.note.subject}</div>
            <div className="modal_texthassold">觀看次數 : {hassold}</div>
            <div className="modal_textprice">售價 : {props.note.price}</div>
            {props.note.pdffile_preview ? (
              <div className="note-preview">
                <div>此筆記提供試閱</div>
                <Button type="primary">
                  <a download={`${props.note.title}_preview.pdf`} href={props.note.pdffile_preview}>
                    試閱筆記
                  </a>
                </Button>
              </div>
            ...
            {props.note.pdffile && (haveBuy == "own" || haveBuy == "buy") ? (
              ...
              <div className="download-note">
                <div>你已擁有此筆記</div>
                <Button type="primary">
                  <a download={`${props.note.title}.pdf`} href={props.note.pdffile}>
                    下載筆記
                  </a>
                </Button>
              </div>
        ...
        <p className="confirm_description">介紹:{props.note.description}</p>
      </Modal>
    </>
  );
};
